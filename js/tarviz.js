...
const slider_margin_left   = 737; // slider position left
...
svg.append('foreignObject').attr("width", 700).attr("height", 30)
            ...
            .style()
            .html("<p>&copy;CC 2018 - HOMMEL & GRAF ENVIRONMENTAL - contact@env.earth</p>");
...
function ready(error, map_db1, map_db2, map_db3, ts_db1, ts_db2, ts_db3, land) {
    ...
    const slider = setup_slider(svg,"#svgTAR",dataObj.unique_years_of_selected_db,uiObj.slider);
    const setup_slider_axis = slider_axis_scale(svg,dataObj.unique_years_of_selected_db);
    //receive slider's initial value (which must be 0):
    const ini_current = document.getElementById('slider_input').value;
    const slider_ini_value = [ini_current];
    ...
    slider.on("change", function(){
        // receive current slider value (index) when moved:
        var current_idx = this.value;


        // + + new method controlling UI object:
        uiObj.slider = current_idx;
        update_viz();
    });
...
const draw_map01 = (volcano, land, svg, allMax) => {
    ...
    // draw land path 1st
    var land = svg.select('g.map').selectAll('path.land').data([land]);
...
const draw_ts = (volcano, thisYear, colors, allMax) => {
    ...
    if (uiObj.buttonAllYears == 'on') {
        ...
        x_axis.call(d3.svg.axis().scale(x).orient("bottom").ticks(15));
    } else {
        const mindate = new Date(thisYear,0,1),
              maxdate = new Date(thisYear,11,31);
        x.domain([mindate, maxdate]);
        x_axis.call(d3.svg.axis().scale(x).orient("bottom").tickFormat(d3.time.format("%B")));
    }

    y.range([ts_height, 0]);
    y.domain([0, 25]);
    y_axis.call(d3.svg.axis().scale(y).orient("left").ticks(6));
    ...
    d3.select('g.ts').append("text")
            ...
            .attr("fill",orange02)
            .attr("font-size",10)
            .attr("transform", "translate(-29,70)rotate(-90)")
            .text("Altitude [km]");
...
function get_time(obj) {
    return d3.time.format.iso.parse(obj["Time Eruption"]);
}



//-- complementary functions - * - * - * - * - * - * - * - * - * - * - * - * - * - * - * - * - * - * - *
function setup_slider(thisSVG,mapID,yearArr,iniVal) {

            const addSlider = d3.select(mapID).append("input")
                    .attr("id", "slider_input")
                    .attr("class", "slider_input_volc")
                    .attr("type", "range")
                    .attr("min", 0)
                    .attr("max", yearArr.length-1)
                    .attr("step", 1)
                    .attr("value", iniVal)
                    .attr("transform", "translate(0,0)");
...
function slider_axis_scale(svg,yearArr) {
            ...
            const slider_scale = d3.scale.ordinal()
                    .domain(yearArr)
                    .rangeRoundBands([0, 600], .1);

            const slider_axis = d3.svg.axis()
                    .scale(slider_scale)
                    .orient("bottom");
...
function map_legend_magnitude_size_circles(thisSVG,thisID,thisScale,){
    ...
    var legText = thisSVG.select('g.legenditems').selectAll('text.legendText')
        ...
        .attr('fill',orange02).attr("font-size","10px")
